...
def draw_staggered_grid_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 35.0
    ...
    # 1. Horizontal Lines (Lower Layer)
    h_mask = (Y % pitch_y) < h_width
    img[in_mat & h_mask] = 80.0

    # 2. Vertical Lines (Upper Layer)
    v_mask = (X % pitch_x) < v_width
    img[in_mat & v_mask] = 120.0

    # 3. Staggered Contacts (Brightest)
    cell_x = (X // pitch_x).astype(np.int32)
    cell_y = (Y // pitch_y).astype(np.int32)
    
    stagger_mask = ((cell_x + cell_y) % 2) == 0

    cx = (X % pitch_x) - (v_width / 2.0)
    cy = (Y % pitch_y) - (h_width / 2.0)
    dist_sq = cx**2 + cy**2
    contact_radius_sq = (15.0 * scale)**2
    
    contact_mask = dist_sq < contact_radius_sq
    img[in_mat & contact_mask & stagger_mask] = 240.0

    # =========================================================================
    # SINGLE GUARANTEED FIDUCIAL (ASYMMETRICAL "T" WITH CUTOUT)
    # Matches reference image: image_9dc11e.png
    # =========================================================================
    gt_phys_x = meta["gt_x"] * 10.0
    gt_phys_y = meta["gt_y"] * 10.0
    
    # 1. Base Dark Pad
    pad_mask = (np.abs(X - gt_phys_x) < 160.0) & (np.abs(Y - gt_phys_y) < 180.0)
    img[pad_mask] = 40.0

    # 2. Faint Outer Border
    border_mask = pad_mask & ((np.abs(X - gt_phys_x) > 140.0) | (np.abs(Y - gt_phys_y) > 160.0))
    img[border_mask] = 65.0
    
    # Convert physical coordinates to local pixel indices for OpenCV drawing
    local_px_x = int((gt_phys_x - start_x_nm) / pixel_scale_nm)
    local_px_y = int((gt_phys_y - start_y_nm) / pixel_scale_nm)
    
    # 3. Draw the bright symbol components using OpenCV
    symbol_mask = np.zeros(img.shape, dtype=np.uint8)
    
    # Vertical Arm
    pt1_v = (local_px_x - 40, local_px_y - 120)
    pt2_v = (local_px_x - 40, local_px_y + 120)
    cv2.line(symbol_mask, pt1_v, pt2_v, 1, thickness=40)
    
    # Horizontal Arm
    pt1_h = (local_px_x - 40, local_px_y + 40)
    pt2_h = (local_px_x + 100, local_px_y + 40)
    cv2.line(symbol_mask, pt1_h, pt2_h, 1, thickness=40)
    
    # Diagonal Arm
    pt1_d = (local_px_x + 40, local_px_y - 60)
    pt2_d = (local_px_x + 100, local_px_y - 120)
    cv2.line(symbol_mask, pt1_d, pt2_d, 1, thickness=25)

    # Apply rounded caps by blurring the hard OpenCV lines before masking
    symbol_mask = cv2.GaussianBlur(symbol_mask * 255, (21, 21), 0)
    img[symbol_mask > 127] = 230.0

    # 4. Black Square Cutout / Via
    cutout_mask = (np.abs(X - (gt_phys_x + 30)) < 22.0) & (np.abs(Y - (gt_phys_y - 60)) < 25.0)
    img[cutout_mask] = 15.0 # Very dark negative space

    return img

def generate_pair(index):
    # FIXED SEED chosen exactly once and hardcoded.
    master_seed = 36553
    np.random.seed(master_seed)
    
    meta = {
        "architecture": "staggered_grid_asym_fiducial",
        "pair_id": index,
        "seed": master_seed,
    ...
    print(f"Generating Deterministic Dataset with Asym Fiducial (Seed: {master_seed})...")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    search_clean_1nm = draw_staggered_grid_layout(
    ...
    ref_clean = draw_staggered_grid_layout(
    ...
    # SAVE TO DISK
    OUTPUT_DIR = Path(__file__).resolve().parent.parent / "results" / "generated_dataset_images" / "dram_37"
...
def main():
    ...
    generate_pair(1)
    print(f"Dataset generation complete! Files saved in {Path(__file__).resolve().parent.parent / 'results' / 'dram_37'}")
