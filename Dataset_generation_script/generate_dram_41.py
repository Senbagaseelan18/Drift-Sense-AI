...
# =========================================================================
# LAYOUT & GEOMETRY ENGINE (3D NAND TRENCHES + UNIQUE SECTIONS + HOLLOW CIRCLE)
# =========================================================================

def draw_3d_nand_unique_sections_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 35.0
    ...
    # =========================================================================
    # 3D NAND FLASH TRENCHES (Tightly packed, slightly wavy vertical lines)
    # =========================================================================
    pitch = 45.0 * scale
    trench_width = 22.0 * scale
    
    # Introduce slight sinusoidal waviness along the vertical lines
    wavy_X = X + np.sin(Y * 0.015) * 6.0
    trench_mask = (wavy_X % pitch) < trench_width
    img[in_mat & trench_mask] = 210.0
    ...
    # =========================================================================
    # SINGLE GUARANTEED FIDUCIAL (CIRCLE WITH ONLY OUTLINE)
    # =========================================================================
    gt_phys_x = meta["gt_x"] * 10.0
    gt_phys_y = meta["gt_y"] * 10.0
    
    # Clear a dark isolation pad beneath the circle so it stands out
    ...
    dy = Y - gt_phys_y
    dist_sq = dx**2 + dy**2
    circle_outline_mask = pad_mask & (dist_sq >= 55**2) & (dist_sq <= 75**2)
    img[circle_outline_mask] = 255.0
...
def generate_pair(index):
    ...
    meta = {
        "architecture": "3d_nand_unique_sections_circle",
        "pair_id": index,
        "seed": master_seed,
        ...
        "gamma": 1.00,
        # 3D NAND is highly susceptible to horizontal charging streaks
        "charging_streaks_per_100_rows": 12.00,
        "charging_streak_intensity": 45.00,
    ...
    print(f"Generating Deterministic Dataset with 3D NAND Trenches & Circle Outline (Seed: {master_seed})...")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    search_clean_1nm = draw_3d_nand_unique_sections_layout(
    ...
    ref_clean = draw_3d_nand_unique_sections_layout(
    ...
    # SAVE TO DISK
    OUTPUT_DIR = Path(__file__).resolve().parent.parent / "results" / "generated_dataset_images" / "dram_41"
...
def main():
    ...
    generate_pair(1)
    print(f"Dataset generation complete! Files saved in {Path(__file__).resolve().parent.parent / 'results' / 'dram_41'}")
