...
OUT = Path(__file__).resolve().parents[1] / "results" / "generated_dataset_images" / "dram_59"
# =========================================================================
...
# =========================================================================
# LAYOUT & GEOMETRY ENGINE (+ & DOT ARRAY WITH "+ IN CIRCLE" SYMBOL)
# =========================================================================

def draw_cross_dot_plus_in_circle_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 35.0
    ...
    dx_val = np.abs(cx - center)
    dy_val = np.abs(cy - center)
    
    # Horizontal pill distance field
    dx_h = np.maximum(dx_val - (arm_len / 2.0 - radius), 0)
    h_pill = (dx_h**2 + dy_val**2) < radius**2

    # Vertical pill distance field
    dy_v = np.maximum(dy_val - (arm_len / 2.0 - radius), 0)
    v_pill = (dx_val**2 + dy_v**2) < radius**2
    ...
    # =========================================================================
    # SINGLE GUARANTEED FIDUCIAL ("+" INSIDE A CIRCLE)
    # =========================================================================
    ...
    pad_mask = (fid_dx < 150.0) & (fid_dy < 150.0)
    img[pad_mask] = 30.0
    
    # Draw hollow circle (Outer Ring)
    circle_mask = pad_mask & (dist_sq >= 85**2) & (dist_sq <= 110**2)
    img[circle_mask] = 255.0
    
    # Draw "+" marker inside the circle
    plus_thickness = 14.0 * scale
    plus_length = 65.0 * scale
    plus_mask = pad_mask & (dist_sq < 80**2) & (((fid_dx <= plus_thickness) & (fid_dy <= plus_length)) | ((fid_dy <= plus_thickness) & (fid_dx <= plus_length)))
...
def generate_pair(index):
    # FIXED SEED hardcoded to 44556 (From requested list)
    master_seed = 44556
    np.random.seed(master_seed)
    
    meta = {
        "architecture": "cross_dot_plus_in_circle",
        "pair_id": index,
        "seed": master_seed,
    ...
    # 200nm buffer inside the mat ensures the 150nm isolation pad doesn't touch the scribe lines
    pad_buffer = 200.0 
    ...
    print(f"Generating Dataset: Strict Internal Placement, '+ in circle' (Seed: {master_seed})...")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    # Generate directly at 1000x1000 with 10 nm/px to avoid expensive 10000x10000 work
    search_clean_1nm = draw_cross_dot_plus_in_circle_layout(
    ...
    ref_clean = draw_cross_dot_plus_in_circle_layout(
...
def main():
    ...
    generate_pair(1)
    print("Dataset generation complete! Files saved in 'dataset_plus_circle_internal'.")
