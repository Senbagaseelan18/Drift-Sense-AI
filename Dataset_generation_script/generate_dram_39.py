...
# =========================================================================
# LAYOUT & GEOMETRY ENGINE WITH SECTION-SPECIFIC UNIQUE FILTERS
# =========================================================================

def draw_sparse_logic_unique_sections_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 35.0
    ...
    # =========================================================================
    # SINGLE GUARANTEED FIDUCIAL (THE SLASH "/")
    # =========================================================================
    gt_phys_x = meta["gt_x"] * 10.0
    gt_phys_y = meta["gt_y"] * 10.0
    
    # Clear a dark isolation pad beneath the slash so it pops clearly against any section style
    pad_mask = (np.abs(X - gt_phys_x) < 100.0) & (np.abs(Y - gt_phys_y) < 100.0)
    img[pad_mask] = 40.0
    
    # Draw the single thick diagonal slash
    dist_to_slash = np.abs((X - gt_phys_x) + (Y - gt_phys_y)) / np.sqrt(2)
    slash_mask = pad_mask & (dist_to_slash < 12.0) & (np.abs(X - gt_phys_x) < 70.0)
...
def generate_pair(index):
    ...
    meta = {
        "architecture": "sparse_logic_unique_sections_slash",
        "pair_id": index,
        "seed": master_seed,
    ...
    print(f"Generating Deterministic Dataset with Unique Sections & Slash (Seed: {master_seed})...")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    search_clean_1nm = draw_sparse_logic_unique_sections_layout(
    ...
    ref_clean = draw_sparse_logic_unique_sections_layout(
    ...
    # SAVE TO DISK
    OUTPUT_DIR = Path(__file__).resolve().parent.parent / "results" / "generated_dataset_images" / "dram_39"
...
def main():
    ...
    generate_pair(1)
    print(f"Dataset generation complete! Files saved in {Path(__file__).resolve().parent.parent / 'results' / 'dram_39'}")
