...
OUT = Path(__file__).resolve().parents[1] / "results" / "generated_dataset_images" / "dram_76"
# =========================================================================
...
# Regional Filter Engine: Applies unique noise profiles to each structural block
def apply_regional_artifacts(img, meta, is_search=True):
    ...
    for bx in range(3):
        for by in range(3):
            ...
            np.random.seed(meta["seed"] + bx * 17 + by * 31)
            
            if is_search:
                contrast = np.random.uniform(0.7, 1.3)
                brightness = np.random.uniform(-30, 30)
                local_dose = np.random.uniform(50.0, 300.0)
                speckle = np.random.uniform(0.0, 0.10)
            else:
                contrast = 1.0
                brightness = 0.0
...
# =========================================================================
# LAYOUT & GEOMETRY ENGINE (MULTI-PATTERN PATCHWORK)
# =========================================================================

def draw_patchwork_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 35.0
    ...
    img[peripheral_mask] = 12.0 # Very dark macro scribe lines

    # Determine which block (0,0 to 2,2) each pixel belongs to
    block_x = (X // period).astype(np.int32)
    block_y = (Y // period).astype(np.int32)
    
    # Hash function to assign 1 of 5 patterns to each block
    pattern_id = (block_x * 7 + block_y * 11) % 5

    # ---------------------------------------------------------
    # PATTERN 0: Classic Dense Circles
    # ---------------------------------------------------------
    p0 = 120.0 * scale
    cx0 = (X % p0) - p0/2.0
    cy0 = (Y % p0) - p0/2.0
    mask0 = (cx0**2 + cy0**2) < (40.0 * scale)**2
    img[in_mat & (pattern_id == 0) & mask0] = 220.0

    # ---------------------------------------------------------
    # PATTERN 1: Checkerboard Squares
    # ---------------------------------------------------------
    p1 = 160.0 * scale
    cx1 = (X // (p1/2.0)).astype(np.int32)
    cy1 = (Y // (p1/2.0)).astype(np.int32)
    mask1 = ((cx1 + cy1) % 2) == 0
    img[in_mat & (pattern_id == 1) & mask1] = 180.0

    # ---------------------------------------------------------
    # PATTERN 2: Vertical Trenches
    # ---------------------------------------------------------
    p2 = 100.0 * scale
    cx2 = (X % p2) - p2/2.0
    mask2 = np.abs(cx2) < (22.0 * scale)
    img[in_mat & (pattern_id == 2) & mask2] = 190.0

    # ---------------------------------------------------------
    # PATTERN 3: Staggered Honeycomb Dots
    # ---------------------------------------------------------
    p3x = 120.0 * scale
    p3y = 200.0 * scale
    c3_1x = (X % p3x) - p3x/2.0
    c3_1y = (Y % p3y) - p3y/2.0
    c3_2x = ((X - p3x/2.0) % p3x) - p3x/2.0
    c3_2y = ((Y - p3y/2.0) % p3y) - p3y/2.0
    mask3 = (c3_1x**2 + c3_1y**2 < (30.0 * scale)**2) | (c3_2x**2 + c3_2y**2 < (30.0 * scale)**2)
    img[in_mat & (pattern_id == 3) & mask3] = 240.0

    # ---------------------------------------------------------
    # PATTERN 4: Diagonal Crosses
    # ---------------------------------------------------------
    p4 = 140.0 * scale
    cx4 = (X % p4) - p4/2.0
    cy4 = (Y % p4) - p4/2.0
    mask4 = (np.abs(cx4 - cy4) < 18.0 * scale) | (np.abs(cx4 + cy4) < 18.0 * scale)
    img[in_mat & (pattern_id == 4) & mask4] = 200.0

    # NO SYMBOLS DRAWN. Relying purely on the patchwork intersection.
    return img

def generate_pair(index):
    # FIXED SEED hardcoded to 80808
    master_seed = 80808
    np.random.seed(master_seed)
    
    meta = {
        "architecture": "patchwork_4way_intersection",
        "pair_id": index,
        "seed": master_seed,
    ...
    # =========================================================================
    # 4-WAY MACRO-INTERSECTION CROP LOGIC
    # =========================================================================
    # By targeting X and Y exactly at the middle of the first scribe lines, 
    # we frame a perfect crosshair showing Block(0,0), (1,0), (0,1), and (1,1).
    
    gt_x_nm = meta["mat_size_nm"] + (meta["strip_width_nm"] / 2.0)
    gt_y_nm = meta["mat_size_nm"] + (meta["strip_width_nm"] / 2.0)
    ...
    print(f"Generating Dataset: Patchwork Sections & 4-Way Intersection Crop (Seed: {master_seed})...")
    print(f"  Reference Crop centered EXACTLY at 4-WAY CROSSROADS: X={meta['gt_x']}px, Y={meta['gt_y']}px")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    # Generate directly at 1000x1000 with 10 nm/px to avoid expensive 10000x10000 work
    search_clean_1nm = draw_patchwork_layout(
    ...
    # Regional Patchwork Filters
    search_final = apply_regional_artifacts(search_drifted, meta, is_search=True)

    # =========================================================================
    # REFERENCE IMAGE GENERATION (Cropped exactly over the 4-way intersection)
    # =========================================================================
    phys_cx = meta["gt_x"] * 10.0
    phys_cy = meta["gt_y"] * 10.0
    
    ref_clean = draw_patchwork_layout(
    ...
    # Clean dose for the Reference Image
...
def main():
    ...
    generate_pair(1)
    print("Dataset generation complete! Files saved in 'dataset_patchwork_4way_intersection'.")
