...
def draw_target_array_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 35.0
    ...
    # =========================================================================
    # SPECIAL METROLOGY TARGET GRID (Matches image_ab0683)
    # =========================================================================
    pitch = 220.0 * scale
    
    cx = X % pitch
    cy = Y % pitch
    dx = np.abs(cx - pitch / 2.0)
    dy = np.abs(cy - pitch / 2.0)

    # 1. Light Gray Base Square
    square_mask = (dx < 55) & (dy < 55)
    img[in_mat & square_mask] = 180.0

    # 2. Inset Faint Square Outline
    inset_outer = (dx < 42) & (dy < 42)
    inset_inner = (dx < 34) & (dy < 34)
    img[in_mat & inset_outer & ~inset_inner] = 150.0

    # 3. Dark Crossbars
    v_arm = (dx < 12) & (dy < 80)
    h_arm = (dy < 12) & (dx < 80)
    img[in_mat & (v_arm | h_arm)] = 60.0
    
    # 4. Rounded Caps for Crossbars
    cap_radius_sq = 12**2
    cap1 = (dx**2 + (dy - 80)**2) < cap_radius_sq  
    cap2 = ((dx - 80)**2 + dy**2) < cap_radius_sq  
    img[in_mat & (cap1 | cap2)] = 60.0
    ...
    # Add a clean dark pad beneath it so the symbol isn't corrupted by the grid
...
def generate_pair(index):
    ...
    # Values perfectly mapped to the UI slider screenshots provided
    meta = {
        "architecture": "metrology_targets_slash",
        "pair_id": index,
        "seed": master_seed,
    ...
    print(f"Generating Deterministic Dataset with '/' Fiducial (Seed: {master_seed})...")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    search_clean_1nm = draw_target_array_layout(
    ...
    ref_clean = draw_target_array_layout(
    ...
    # SAVE TO DISK
    OUTPUT_DIR = Path(__file__).resolve().parent.parent / "results" / "generated_dataset_images" / "dram_35"
...
def main():
    ...
    generate_pair(1)
    print(f"Dataset generation complete! Files saved in {Path(__file__).resolve().parent.parent / 'results' / 'dram_35'}")
