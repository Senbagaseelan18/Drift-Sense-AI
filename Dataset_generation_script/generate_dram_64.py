...
OUT = Path(__file__).resolve().parents[1] / "results" / "generated_dataset_images" / "dram_64"
# =========================================================================
...
# =========================================================================
# LAYOUT & GEOMETRY ENGINE (HEXAGONAL ARRAY + BULLSEYE SYMBOL)
# =========================================================================

def draw_hexagonal_bullseye_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 35.0
    ...
    # =========================================================================
    # HEXAGONAL / HONEYCOMB ARRAY (Staggered Circles)
    # =========================================================================
    pitch = 120.0 * scale
    h_pitch = pitch * np.sqrt(3) / 2.0
    radius_sq = (40.0 * scale)**2
    
    # Primary Grid
    cx1 = (X % pitch) - (pitch / 2.0)
    cy1 = (Y % (2 * h_pitch)) - h_pitch
    dist1 = cx1**2 + cy1**2
    
    # Offset/Staggered Grid
    cx2 = ((X - pitch/2.0) % pitch) - (pitch / 2.0)
    cy2 = ((Y - h_pitch) % (2 * h_pitch)) - h_pitch
    dist2 = cx2**2 + cy2**2
    
    hex_mask = (dist1 < radius_sq) | (dist2 < radius_sq)
    img[in_mat & hex_mask] = 175.0
    
    # Small dark core inside each contact for texture
    core_mask = (dist1 < (15.0 * scale)**2) | (dist2 < (15.0 * scale)**2)
    img[in_mat & core_mask] = 60.0

    # =========================================================================
    # SINGLE GUARANTEED FIDUCIAL (CONCENTRIC RINGS / BULLSEYE)
    # =========================================================================
    ...
    # Outer Ring
    ring1 = pad_mask & (dist_sq > (75.0 * scale)**2) & (dist_sq < (95.0 * scale)**2)
    # Inner Ring
    ring2 = pad_mask & (dist_sq > (35.0 * scale)**2) & (dist_sq < (55.0 * scale)**2)
    # Core Dot
    core_dot = pad_mask & (dist_sq < (15.0 * scale)**2)
    
    img[ring1 | ring2 | core_dot] = 255.0

    return img

def generate_pair(index):
    # BRAND NEW SEED for a different grid location
    master_seed = 86420
    np.random.seed(master_seed)
    
    meta = {
        "architecture": "hexagonal_array_bullseye",
        "pair_id": index,
        "seed": master_seed,
        ...
        # Astigmatism applied to warp the bullseye slightly
        "beam_spot_size_nm": 4.50,              
        "beam_astigmatism_ratio": 1.25,         # Warps circles into ellipses
        "barrel_distortion": 0.00,              
        "vignette_strength": 0.15,              
        "gamma": 0.90,                          
        "charging_streaks_per_100_rows": 15.00, 
    ...
    print(f"Generating Dataset: Hexagonal Array & Bullseye (Seed: {master_seed})...")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    # Generate directly at 1000x1000 with 10 nm/px to avoid expensive 10000x10000 work
    search_clean_1nm = draw_hexagonal_bullseye_layout(
    ...
    ref_clean = draw_hexagonal_bullseye_layout(
...
def main():
    ...
    generate_pair(1)
    print("Dataset generation complete! Files saved in 'dataset_hexagonal_bullseye'.")
