...
OUT = Path(__file__).resolve().parents[1] / "results" / "generated_dataset_images" / "dram_75"
# =========================================================================
...
def draw_sparse_random_logic_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    ...
    # 3. Sparse Vias connecting M1/M2 intersections (Drop density to ~45%)
    via_active = via_hash < 45
    dist_sq = (cx - pitch/2)**2 + (cy - pitch/2)**2
    via_mask = dist_sq < via_radius**2
    
    # Only place vias where traces might intersect or terminate
    img[in_mat & via_mask & via_active] = 255.0

    # ZERO SYMBOLS RENDERED. The reference image is strictly the generated maze pattern.

    return img

def generate_pair(index):
    # FIXED SEED hardcoded to 77889
    master_seed = 77889
    np.random.seed(master_seed)
    
    meta = {
        "architecture": "sparse_via_maze_no_symbol",
        "pair_id": index,
        "seed": master_seed,
    ...
    # PLACEMENT LOGIC
    # =========================================================================
    # We still need a ground truth coordinate to crop the reference image out of the search image
    ...
    print(f"Generating Dataset: Sparse Random Logic (No Symbols) (Seed: {master_seed})...")
    print(f"  Cropping Reference at Ground Truth: X={meta['gt_x']}, Y={meta['gt_y']}")
    ...
    # =========================================================================
    # REFERENCE IMAGE GENERATION (Cropped directly from the maze geometry)
    # =========================================================================
...
def main():
    ...
    generate_pair(1)
    print("Dataset generation complete! Files saved in 'dataset_sparse_logic_no_symbols'.")
