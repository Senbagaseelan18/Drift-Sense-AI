...
OUT = Path(__file__).resolve().parents[1] / "results" / "generated_dataset_images" / "dram_60"
# =========================================================================
...
# =========================================================================
# LAYOUT & GEOMETRY ENGINE (SRAM BUTTERFLY CELLS + VERNIER COMB)
# =========================================================================

def draw_sram_vernier_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 35.0
    ...
    # Base Substrate (N-Well / P-Well visual differentiation)
    well_mask = (cx < pitch_x / 2.0)
    img[in_mat & well_mask] = 45.0
    
    # 1. Active Regions (Diffusion) - The interlocking L/T shapes
    diff_1 = (cx > 20.0*scale) & (cx < 100.0*scale) & (cy > 20.0*scale) & (cy < 80.0*scale)
    diff_2 = (cx > 140.0*scale) & (cx < 220.0*scale) & (cy > 280.0*scale) & (cy < 340.0*scale)
    diff_3 = (cx > 80.0*scale) & (cx < 120.0*scale) & (cy > 80.0*scale) & (cy < 340.0*scale)
    diff_4 = (cx > 120.0*scale) & (cx < 160.0*scale) & (cy > 20.0*scale) & (cy < 280.0*scale)
    active_mask = diff_1 | diff_2 | diff_3 | diff_4
    img[in_mat & active_mask] = 170.0

    # 2. Polysilicon Gates (Vertical crossing traces)
    poly_1 = (cx > 40.0*scale) & (cx < 60.0*scale) & (cy > 40.0*scale) & (cy < 280.0*scale)
    poly_2 = (cx > 180.0*scale) & (cx < 200.0*scale) & (cy > 80.0*scale) & (cy < 320.0*scale)
    poly_mask = poly_1 | poly_2
    img[in_mat & poly_mask] = 110.0

    # 3. Via / Contacts (Bright centers on diffusion nodes)
    contact_1 = (np.abs(cx - 100.0*scale) < 12.0*scale) & (np.abs(cy - 50.0*scale) < 12.0*scale)
    contact_2 = (np.abs(cx - 140.0*scale) < 12.0*scale) & (np.abs(cy - 310.0*scale) < 12.0*scale)
    contact_mask = contact_1 | contact_2
    img[in_mat & contact_mask] = 245.0

    # =========================================================================
    # VERNIER COMB METROLOGY STRUCTURE
    # =========================================================================
    gt_phys_x = meta["gt_x"] * 10.0
    gt_phys_y = meta["gt_y"] * 10.0
    
    local_cx = X - gt_phys_x
    local_cy = Y - gt_phys_y
    
    # Isolation pad
    pad_mask = (np.abs(local_cx) < 160.0) & (np.abs(local_cy) < 160.0)
    img[pad_mask] = 28.0
    
    # 1. Top Comb Half (Pitch = 20nm)
    top_pitch = 20.0 * scale
    top_comb_area = pad_mask & (local_cy < -15.0) & (local_cy > -110.0) & (np.abs(local_cx) < 130.0)
    # np.abs + modulo naturally centers the zero-phase tooth precisely at x=0
    top_teeth = (np.abs(local_cx + (top_pitch/2.0)) % top_pitch) < (6.0 * scale)
    img[top_comb_area & top_teeth] = 255.0
    
    # 2. Bottom Comb Half (Pitch = 22nm - slight mismatch for visual phase tracking)
    bot_pitch = 22.0 * scale
    bot_comb_area = pad_mask & (local_cy > 15.0) & (local_cy < 110.0) & (np.abs(local_cx) < 130.0)
    bot_teeth = (np.abs(local_cx + (bot_pitch/2.0)) % bot_pitch) < (6.0 * scale)
    img[bot_comb_area & bot_teeth] = 255.0

    # 3. Center Separation Bar
    center_bar = pad_mask & (np.abs(local_cy) <= 4.0) & (np.abs(local_cx) < 140.0)
    img[center_bar] = 180.0

    return img

def generate_pair(index):
    # FIXED SEED hardcoded to 11223 (From requested list)
    master_seed = 11223
    np.random.seed(master_seed)
    
    meta = {
        "architecture": "sram_butterfly_vernier_comb",
        "pair_id": index,
        "seed": master_seed,
        ...
        "gamma": 1.00,
        "charging_streaks_per_100_rows": 2.00,
    ...
    # Randomly select one of the fully enclosed blocks (0, 1, or 2) in the 10x10um grid
    block_x = np.random.randint(0, 3)
    block_y = np.random.randint(0, 3)
    
    # 200nm buffer inside the mat ensures the 160nm isolation pad doesn't touch the scribe lines
    pad_buffer = 200.0 
    ...
    print(f"Generating Dataset: SRAM Layout & Vernier Comb (Seed: {master_seed})...")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    # Generate directly at 1000x1000 with 10 nm/px to avoid expensive 10000x10000 work
    search_clean_1nm = draw_sram_vernier_layout(
    ...
    ref_clean = draw_sram_vernier_layout(
...
def main():
    ...
    generate_pair(1)
    print("Dataset generation complete! Files saved in 'dataset_sram_vernier'.")
