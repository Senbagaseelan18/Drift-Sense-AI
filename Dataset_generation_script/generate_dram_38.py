...
def draw_sparse_logic_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 35.0
    ...
    img[peripheral_mask] = 25.0 # Dark scribe lines
    ...
    # Grid coordinate hashing for procedural generation
    ...
    # Draw segment if hash < 45 (45% density)
    h_active = h_hash < 45
    ...
    # 1. Clear a dark isolation pad around the marker
    pad_mask = (fid_dx < 220.0) & (fid_dy < 220.0)
    img[pad_mask] = 30.0
    
    # 2. Outer Box (Hollow Square)
    outer_box_out = (fid_dx < 160.0) & (fid_dy < 160.0)
    outer_box_in = (fid_dx < 120.0) & (fid_dy < 120.0)
    img[pad_mask & outer_box_out & ~outer_box_in] = 180.0
    
    # 3. Inner Box (Hollow Square)
    inner_box_out = (fid_dx < 70.0) & (fid_dy < 70.0)
    inner_box_in = (fid_dx < 30.0) & (fid_dy < 30.0)
    img[pad_mask & inner_box_out & ~inner_box_in] = 240.0

    return img

def generate_pair(index):
    # FIXED SEED hardcoded directly into the script for determinism
    master_seed = 77301
    np.random.seed(master_seed)
    
    meta = {
        "architecture": "sparse_logic_bib",
        "pair_id": index,
        "seed": master_seed,
    ...
    print(f"Generating Deterministic Dataset with BiB Fiducial (Seed: {master_seed})...")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    search_clean_1nm = draw_sparse_logic_layout(
    ...
    ref_clean = draw_sparse_logic_layout(
    ...
    # SAVE TO DISK
    OUTPUT_DIR = Path(__file__).resolve().parent.parent / "results" / "generated_dataset_images" / "dram_38"
...
def main():
    ...
    generate_pair(1)
    print(f"Dataset generation complete! Files saved in {Path(__file__).resolve().parent.parent / 'results' / 'dram_38'}")
