...
OUT = Path(__file__).resolve().parents[1] / "results" / "generated_dataset_images" / "dram_62"
# =========================================================================
# PHYSICS & ARTIFACT ENGINES (Realistic SEM Application)
# =========================================================================
...
# =========================================================================
# LAYOUT & GEOMETRY ENGINE (REPEATING "X" ARRAY + "X IN SQUARE" SYMBOL)
# =========================================================================

def draw_x_array_square_x_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 35.0
    ...
    # Distance to the two diagonal axes within each cell
    dist_main = np.abs(cx - cy) / np.sqrt(2)
    dist_anti = np.abs(cx + cy - pitch) / np.sqrt(2)
    
    thickness = 12.0 * scale
    x_array_mask = (dist_main < thickness) | (dist_anti < thickness)
    
    # Add a gap around the edges of the cell so the X's don't touch, creating distinct shapes
    cell_bounds = (cx > 15.0 * scale) & (cx < pitch - 15.0 * scale) & (cy > 15.0 * scale) & (cy < pitch - 15.0 * scale)
    
    img[in_mat & x_array_mask & cell_bounds] = 160.0

    # Add small bright contacts in the exact center of each X
    contact_mask = (np.abs(cx - center) < 8.0 * scale) & (np.abs(cy - center) < 8.0 * scale)
    img[in_mat & contact_mask & cell_bounds] = 230.0

    # =========================================================================
    # SINGLE GUARANTEED FIDUCIAL ("X" INSIDE A SQUARE)
    # =========================================================================
    ...
    # 1. Clear dark isolation pad
    pad_mask = (fid_dx < 160.0) & (fid_dy < 160.0)
    img[pad_mask] = 28.0
    
    # 2. Draw Outer Square
    square_out = (fid_dx <= 120.0) & (fid_dy <= 120.0)
    square_in = (fid_dx < 100.0) & (fid_dy < 100.0)
    img[pad_mask & square_out & ~square_in] = 200.0
    
    # 3. Draw "X" inside the square
    fid_dist_main = np.abs((X - gt_phys_x) - (Y - gt_phys_y)) / np.sqrt(2)
    fid_dist_anti = np.abs((X - gt_phys_x) + (Y - gt_phys_y)) / np.sqrt(2)
    
    fid_x_thickness = 14.0 * scale
    inner_x_mask = pad_mask & square_in & ((fid_dist_main < fid_x_thickness) | (fid_dist_anti < fid_x_thickness)) & (fid_dx < 75.0) & (fid_dy < 75.0)
    
    img[inner_x_mask] = 255.0
...
def generate_pair(index):
    ...
    meta = {
        "architecture": "x_array_square_x_fiducial",
        "pair_id": index,
        "seed": master_seed,
        ...
        # Standard SEM Physics
        "beam_spot_size_nm": 4.00,
    ...
    print(f"Generating Dataset: 'X' Array & 'X in Square' (Seed: {master_seed})...")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    # Generate directly at 1000x1000 with 10 nm/px to avoid expensive 10000x10000 work
    search_clean_1nm = draw_x_array_square_x_layout(
    ...
    ref_clean = draw_x_array_square_x_layout(
...
def main():
    ...
    generate_pair(1)
    print("Dataset generation complete! Files saved in 'dataset_x_array_square_x'.")
