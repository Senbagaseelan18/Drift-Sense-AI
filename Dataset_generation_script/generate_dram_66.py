...
OUT = Path(__file__).resolve().parents[1] / "results" / "generated_dataset_images" / "dram_66"
# =========================================================================
...
# =========================================================================
# LAYOUT & GEOMETRY ENGINE (HOLLOW CIRCLES + "L" SYMBOL)
# =========================================================================

def draw_hollow_circles_l_symbol_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 35.0
    ...
    # =========================================================================
    # HOLLOW CIRCLE OUTLINE ARRAY
    # =========================================================================
    pitch = 140.0 * scale
    cx = (X % pitch) - (pitch / 2.0)
    cy = (Y % pitch) - (pitch / 2.0)
    dist_sq_circles = cx**2 + cy**2
    
    outer_radius_sq = (50.0 * scale)**2
    inner_radius_sq = (35.0 * scale)**2
    
    # Render only the outline of the circles
    hollow_circle_mask = (dist_sq_circles >= inner_radius_sq) & (dist_sq_circles <= outer_radius_sq)
    img[in_mat & hollow_circle_mask] = 190.0

    # =========================================================================
    # SINGLE GUARANTEED FIDUCIAL ("L" SYMBOL)
    # =========================================================================
    ...
    pad_mask = (np.abs(fid_dx) < 130.0 * scale) & (np.abs(fid_dy) < 130.0 * scale)
    img[pad_mask] = 30.0
    
    # 2. Draw the "L" Shape
    # Vertical Bar (Left side)
    vert_bar = pad_mask & (fid_dx > -45.0 * scale) & (fid_dx < -15.0 * scale) & (fid_dy > -60.0 * scale) & (fid_dy < 60.0 * scale)
    # Horizontal Bar (Bottom)
    horz_bar = pad_mask & (fid_dx > -45.0 * scale) & (fid_dx < 55.0 * scale) & (fid_dy > 30.0 * scale) & (fid_dy < 60.0 * scale)
...
def generate_pair(index):
    # FIXED SEED hardcoded to 13579
    master_seed = 13579
    np.random.seed(master_seed)
    
    meta = {
        "architecture": "hollow_circles_l_symbol",
        "pair_id": index,
        "seed": master_seed,
    ...
    # =========================================================================
    # STRICT MAT-INTERNAL PLACEMENT LOGIC (TOP OF SECTION)
    # =========================================================================
    ...
    # Calculate global X boundaries for the chosen mat
    min_x = (block_x * period) + pad_buffer
    max_x = (block_x * period) + meta["mat_size_nm"] - pad_buffer
    
    # Calculate global Y boundaries for the chosen mat, forcing it to the TOP 25% of the block
    min_y = (block_y * period) + pad_buffer
    max_y_top_limit = min_y + (meta["mat_size_nm"] * 0.25)
    
    gt_x_nm = np.random.uniform(min_x, max_x)
    gt_y_nm = np.random.uniform(min_y, max_y_top_limit)
    ...
    print(f"Generating Dataset: Hollow Circles & Top-Aligned 'L' Symbol (Seed: {master_seed})...")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    # Generate directly at 1000x1000 with 10 nm/px to avoid expensive 10000x10000 work
    search_clean_1nm = draw_hollow_circles_l_symbol_layout(
    ...
    ref_clean = draw_hollow_circles_l_symbol_layout(
...
def main():
    ...
    generate_pair(1)
    print("Dataset generation complete! Files saved in 'dataset_hollow_circles_l_symbol'.")
