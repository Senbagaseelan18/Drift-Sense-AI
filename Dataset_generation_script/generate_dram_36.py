...
def draw_staggered_grid_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    ...
    # =========================================================================
    # SINGLE GUARANTEED FIDUCIAL (THE HOLLOW TRIANGLE)
    # =========================================================================
    gt_phys_x = meta["gt_x"] * 10.0
    gt_phys_y = meta["gt_y"] * 10.0
    
    # 1. Add a clean dark pad beneath it
    pad_mask = (np.abs(X - gt_phys_x) < 180.0) & (np.abs(Y - gt_phys_y) < 180.0)
    img[pad_mask] = 40.0
    
    # 2. Convert physical coordinates to local pixel indices for OpenCV drawing
    local_px_x = int((gt_phys_x - start_x_nm) / pixel_scale_nm)
    local_px_y = int((gt_phys_y - start_y_nm) / pixel_scale_nm)
    
    # 3. Draw the hollow triangle using OpenCV Polylines
    tri_pts = np.array([
        [local_px_x, local_px_y - 120],      
        [local_px_x - 140, local_px_y + 100], 
        [local_px_x + 140, local_px_y + 100]  
    ], np.int32).reshape((-1, 1, 2))
    
    triangle_mask = np.zeros(img.shape, dtype=np.uint8)
    cv2.polylines(triangle_mask, [tri_pts], isClosed=True, color=1, thickness=18)
    
    img[triangle_mask == 1] = 255.0
...
def generate_pair(index):
    ...
    meta = {
        "architecture": "staggered_grid_triangle",
        "pair_id": index,
        "seed": master_seed,
    ...
    print(f"Generating Deterministic Dataset with Triangle Fiducial (Seed: {master_seed})...")
    ...
    # SAVE TO DISK
    OUTPUT_DIR = Path(__file__).resolve().parent.parent / "results" / "generated_dataset_images" / "dram_36"
...
def main():
    ...
    generate_pair(1)
    print(f"Dataset generation complete! Files saved in {Path(__file__).resolve().parent.parent / 'results' / 'dram_36'}")
