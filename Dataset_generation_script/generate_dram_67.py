...
OUT = Path(__file__).resolve().parents[1] / "results" / "generated_dataset_images" / "dram_67"
# =========================================================================
...
# =========================================================================
# LAYOUT & GEOMETRY ENGINE (STAGGERED DOTS + TWO "/" SYMBOLS)
# =========================================================================

def draw_staggered_dots_double_slash_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 25.0  # Very dark background based on image_3b28fe.png
    ...
    # =========================================================================
    # STAGGERED DOT ARRAY (Hexagonal/closest-packed layout)
    # =========================================================================
    pitch_x = 180.0 * scale
    pitch_y = 155.0 * scale
    ...
    dist1_sq = cx1**2 + cy1**2
    
    # Grid 2 (Offset by half pitch X and full pitch Y)
    cx2 = ((X - pitch_x/2.0) % pitch_x) - (pitch_x / 2.0)
    cy2 = ((Y - pitch_y) % (2 * pitch_y)) - pitch_y
    dist2_sq = cx2**2 + cy2**2
    
    outer_radius_sq = (30.0 * scale)**2
    inner_radius_sq = (15.0 * scale)**2
    
    # Draw outer gray circles
    outer_dot_mask = (dist1_sq < outer_radius_sq) | (dist2_sq < outer_radius_sq)
    img[in_mat & outer_dot_mask] = 190.0
    
    # Draw inner bright cores (giving the 3D/beveled look from the reference)
    inner_dot_mask = (dist1_sq < inner_radius_sq) | (dist2_sq < inner_radius_sq)
    img[in_mat & inner_dot_mask] = 250.0

    # =========================================================================
    # MULTIPLE GUARANTEED FIDUCIALS (TWO "/" SYMBOLS)
    # =========================================================================
    # Loop over both ground truth locations defined in meta
    for gx, gy in [(meta["gt_x"] * 10.0, meta["gt_y"] * 10.0), 
        ...
        # "/" Slash Symbol
        dist_to_slash = np.abs(fid_dx + fid_dy) / np.sqrt(2)
        slash_mask = pad_mask & (dist_to_slash < 14.0 * scale) & (np.abs(fid_dx) < 70.0 * scale) & (np.abs(fid_dy) < 70.0 * scale)
        img[slash_mask] = 255.0

    return img

def generate_pair(index):
    # FIXED SEED hardcoded to 97531
    master_seed = 97531
    np.random.seed(master_seed)
    
    meta = {
        "architecture": "staggered_dots_double_slash",
        "pair_id": index,
        "seed": master_seed,
        ...
        "dose_search": 200.00,                  
        "shear_amplitude_px": 1.20,             
        "drift_jitter_px": 0.40,                
        "speckle_noise_sigma": 0.04,            
        "salt_and_pepper_prob": 0.005           
    }

    # =========================================================================
    # STRICT MAT-INTERNAL PLACEMENT LOGIC FOR TWO TARGETS
    # =========================================================================
    period = meta["mat_size_nm"] + meta["strip_width_nm"]
    
    # Pick two distinct blocks on the 3x3 layout to ensure the pads don't overlap
    available_blocks = [(x, y) for x in range(3) for y in range(3)]
    np.random.shuffle(available_blocks)
    block1, block2 = available_blocks[0], available_blocks[1]
    
    pad_buffer = 200.0 
    
    # Target 1 (Primary Ground Truth - used for Reference Image framing)
    min_x1 = (block1[0] * period) + pad_buffer
    max_x1 = (block1[0] * period) + meta["mat_size_nm"] - pad_buffer
    min_y1 = (block1[1] * period) + pad_buffer
    max_y1 = (block1[1] * period) + meta["mat_size_nm"] - pad_buffer
    ...
    # Target 2 (Secondary Target - drawn in the Search Image, but off-center)
    min_x2 = (block2[0] * period) + pad_buffer
    max_x2 = (block2[0] * period) + meta["mat_size_nm"] - pad_buffer
    min_y2 = (block2[1] * period) + pad_buffer
    max_y2 = (block2[1] * period) + meta["mat_size_nm"] - pad_buffer
    ...
    print(f"Generating Dataset: Staggered Dots & Double Slashes (Seed: {master_seed})...")
    print(f"  Target 1 located at Block {block1}")
    print(f"  Target 2 located at Block {block2}")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    # Generate directly at 1000x1000 with 10 nm/px to avoid expensive 10000x10000 work
    search_clean_1nm = draw_staggered_dots_double_slash_layout(
    ...
    # =========================================================================
    # REFERENCE IMAGE GENERATION (Centered on Target 1)
    # =========================================================================
    phys_cx = meta["gt_x"] * 10.0
    phys_cy = meta["gt_y"] * 10.0
    
    ref_clean = draw_staggered_dots_double_slash_layout(
...
def main():
    ...
    generate_pair(1)
    print("Dataset generation complete! Files saved in 'dataset_staggered_dots_double_slash'.")
