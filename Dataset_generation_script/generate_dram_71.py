...
OUT = Path(__file__).resolve().parents[1] / "results" / "generated_dataset_images" / "dram_71"
# =========================================================================
...
# =========================================================================
# LAYOUT & GEOMETRY ENGINE (SQUARE ARRAY + PLUS IN WHITE BOX)
# =========================================================================

def draw_squares_white_box_plus_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 30.0
    ...
    img[peripheral_mask] = 18.0 # Dark scribe lines for the edges
    ...
    # Draw simple squares
    square_size = 40.0 * scale
    sq_mask = (np.abs(cx) < square_size) & (np.abs(cy) < square_size)
    
    img[in_mat & sq_mask] = 195.0

    # =========================================================================
    # TWO GUARANTEED FIDUCIALS ("+" IN A WHITE BOX)
    # =========================================================================
    for gx, gy in [(meta["gt_x"] * 10.0, meta["gt_y"] * 10.0), 
        ...
        # 1. Dark Isolation Pad (To ensure contrast when overlapping edges)
        pad_mask = (np.abs(fid_dx) < 140.0 * scale) & (np.abs(fid_dy) < 140.0 * scale)
        img[pad_mask] = 25.0
        
        # 2. White Box Outline
        box_out = pad_mask & (np.abs(fid_dx) < 100.0 * scale) & (np.abs(fid_dy) < 100.0 * scale)
        box_in = pad_mask & (np.abs(fid_dx) < 75.0 * scale) & (np.abs(fid_dy) < 75.0 * scale)
        img[box_out & ~box_in] = 255.0
        
        # 3. Inner "+" Symbol
        plus_thickness = 18.0 * scale
        plus_length = 45.0 * scale
        
        vert_bar = pad_mask & (np.abs(fid_dx) < (plus_thickness / 2.0)) & (np.abs(fid_dy) < plus_length)
        horz_bar = pad_mask & (np.abs(fid_dy) < (plus_thickness / 2.0)) & (np.abs(fid_dx) < plus_length)
        
        img[vert_bar | horz_bar] = 255.0

    return img

def generate_pair(index):
    # FIXED SEED hardcoded to 35791
    master_seed = 35791
    np.random.seed(master_seed)
    
    meta = {
        "architecture": "squares_white_box_plus_edge",
        "pair_id": index,
        "seed": master_seed,
    ...
    # =========================================================================
    # EXTREME EDGE STRADDLING LOGIC (OVERLAPPING THE BORDERS)
    # =========================================================================
    period = meta["mat_size_nm"] + meta["strip_width_nm"]
    
    # Target 1 (Primary Ground Truth) -> Forced onto a VERTICAL EDGE
    # Placed on the vertical scribe line between Block (0,y) and Block (1,y)
    ...
    # Placed on the horizontal scribe line between Block (x,0) and Block (x,1)
    gt2_x_nm = period + (period / 2.0) # Center of the block horizontally
    gt2_y_nm = period - (meta["strip_width_nm"] / 2.0)
    
    meta["gt2_x"] = round(gt2_x_nm / 10.0, 2)
    meta["gt2_y"] = round(gt2_y_nm / 10.0, 2)

    print(f"Generating Dataset: Square Array & Overlapping Edge Symbols (Seed: {master_seed})...")
    print(f"  Target 1 located OVERLAPPING VERTICAL EDGE at X={meta['gt_x']}px")
    print(f"  Target 2 located OVERLAPPING HORIZONTAL EDGE at Y={meta['gt2_y']}px")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    # Generate directly at 1000x1000 with 10 nm/px to avoid expensive 10000x10000 work
    search_clean_1nm = draw_squares_white_box_plus_layout(
    ...
    ref_clean = draw_squares_white_box_plus_layout(
...
def main():
    ...
    generate_pair(1)
    print("Dataset generation complete! Files saved in 'dataset_squares_white_box_plus_edges'.")
