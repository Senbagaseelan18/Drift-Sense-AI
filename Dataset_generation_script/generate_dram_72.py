...
OUT = Path(__file__).resolve().parents[1] / "results" / "generated_dataset_images" / "dram_72"
# =========================================================================
...
# =========================================================================
# LAYOUT & GEOMETRY ENGINE (SPARSE RANDOM LOGIC / VIA MAZE - NO SYMBOLS)
# =========================================================================

def draw_sparse_random_logic_layout(width_px, height_px, pixel_scale_nm, start_x_nm, start_y_nm, meta):
    BACKGROUND = 35.0
    ...
    img[peripheral_mask] = 18.0 # Dark scribe lines

    # =========================================================================
    # SPARSE RANDOM LOGIC (The Via Maze)
    # =========================================================================
    pitch = 120.0 * scale
    trace_width = 30.0 * scale
    via_radius = 22.0 * scale
    
    cell_x = (X // pitch).astype(np.int32)
    cell_y = (Y // pitch).astype(np.int32)
    cx = X % pitch
    cy = Y % pitch
    
    # Deterministic hashing to decide if a cell has M1, M2, and Vias
    h_hash = (cell_x * 31337 + cell_y * 8121 + seed) % 100
    v_hash = (cell_x * 12345 + cell_y * 67890 + seed) % 100
    via_hash = (cell_x * 9991 + cell_y * 4447 + seed) % 100

    # 1. Random Horizontal Routing (M1)
    h_active = h_hash < 50
    h_mask = (cy > (pitch/2 - trace_width/2)) & (cy < (pitch/2 + trace_width/2))
    img[in_mat & h_mask & h_active] = 90.0

    # 2. Random Vertical Routing (M2)
    v_active = v_hash < 50
    v_mask = (cx > (pitch/2 - trace_width/2)) & (cx < (pitch/2 + trace_width/2))
    img[in_mat & v_mask & v_active] = 130.0

    # 3. Sparse Vias connecting M1/M2 intersections
    via_active = via_hash < 45
    dist_sq = (cx - pitch/2)**2 + (cy - pitch/2)**2
    via_mask = dist_sq < via_radius**2
    
    img[in_mat & via_mask & via_active] = 255.0

    # NO FIDUCIAL SYMBOLS DRAWN

    return img

def generate_pair(index):
    # FIXED SEED hardcoded to 65432
    master_seed = 65432
    np.random.seed(master_seed)
    
    meta = {
        "architecture": "sparse_via_maze_boundary_crop",
        "pair_id": index,
        "seed": master_seed,
        ...
        # Balanced SEM Filters for texture without destroying edges
    ...
    # =========================================================================
    # BOUNDARY STRADDLING LOGIC (CENTERING REF IMAGE ON SCRIBE LINE)
    # =========================================================================
    # We will force the Ground Truth (which dictates the 1000x1000px Reference crop) 
    # to perfectly center on the first vertical scribe line separating two sections.
    
    # Place X exactly in the center of the first vertical street
    ...
    print(f"Generating Dataset: Via Maze with Boundary Crop (Seed: {master_seed})...")
    print(f"  Reference Crop centered exactly on VERTICAL BOUNDARY at X={meta['gt_x']}px")

    # =========================================================================
    # SEARCH IMAGE GENERATION
    # =========================================================================
    # Generate directly at 1000x1000 with 10 nm/px to avoid expensive 10000x10000 work
    search_clean_1nm = draw_sparse_random_logic_layout(
    ...
    # =========================================================================
    # REFERENCE IMAGE GENERATION (Cropped exactly over the boundary)
    # =========================================================================
    phys_cx = meta["gt_x"] * 10.0
    phys_cy = meta["gt_y"] * 10.0
    
    ref_clean = draw_sparse_random_logic_layout(
...
def main():
    ...
    generate_pair(1)
    print("Dataset generation complete! Files saved in 'dataset_sparse_logic_boundary'.")
